...
async function cargarDatosAdmin() {
    showLoading();
    try {
        var resp1 = await supabase.from('ventas').select('*').order('fecha', { ascending: false }).order('hora', { ascending: false });
...
function actualizarDashboard() {
    ...
    // TURNOS - Mañana: 10-16h, Tarde: 16-24h
    ...
    console.log('Turnos HOY - Mañana:', ventasManana.length, 'Tarde:', ventasTarde.length);
...
function mostrarListaClientes() {
    var html = '';
    for (var i = 0; i < todosLosClientes.length; i++) { var c = todosLosClientes[i]; var bono = (parseFloat(c.bono_total) || 0) - (parseFloat(c.bono_gastado) || 0); html += '<div class="cliente-mini" onclick="buscarClientePorNumero(' + c.numero + ')"><span class="cliente-mini-num">#' + c.numero + '</span><span class="cliente-mini-nombre">' + (c.nombre || 'Sin nombre') + '</span><span class="cliente-mini-bono">' + formatMoney(bono) + '</span></div>'; }
...
function mostrarListaVentas() {
    ...
    for (var i = 0; i < Math.min(ventas.length, 200); i++) {
        ...
        html += '<div class="venta-admin-item"><span class="venta-fecha">' + formatDate(v.fecha) + ' ' + hora + '</span><span class="venta-cliente">#' + v.numero_cliente + '</span><span class="venta-desc">' + (v.descripcion || '-') + '</span><span class="venta-precio">' + formatMoney(v.precio) + '</span><button class="venta-delete" onclick="borrarVentaAdmin(' + v.id + ')">🗑️</button></div>';
    }
