...
function formatMoney(amount) { return (parseFloat(amount) || 0).toFixed(2).replace('.', ',') + ' EUR'; }
function formatDate(dateStr) { if (!dateStr) return '--'; var parts = dateStr.split('-'); if (parts.length === 3) return parts[2] + '/' + parts[1]; return dateStr; }
function formatDateFull(dateStr) { if (!dateStr) return '--'; var parts = dateStr.split('-'); if (parts.length === 3) return parts[2] + '/' + parts[1] + '/' + parts[0]; return dateStr; }
function formatHora(horaStr) { if (!horaStr) return ''; return horaStr.substring(0, 5); }
function showScreen(screen) { loginScreen.classList.remove('active'); mainScreen.classList.remove('active'); adminScreen.classList.remove('active'); screen.classList.add('active'); }
function getHoy() { var d = new Date(); return d.getFullYear() + '-' + (d.getMonth()+1).toString().padStart(2,'0') + '-' + d.getDate().toString().padStart(2,'0'); }
function getAyer() { var d = new Date(); d.setDate(d.getDate() - 1); return d.getFullYear() + '-' + (d.getMonth()+1).toString().padStart(2,'0') + '-' + d.getDate().toString().padStart(2,'0'); }
function getInicioSemana() { var d = new Date(); var day = d.getDay(); var diff = d.getDate() - day + (day === 0 ? -6 : 1); d.setDate(diff); return d.getFullYear() + '-' + (d.getMonth()+1).toString().padStart(2,'0') + '-' + d.getDate().toString().padStart(2,'0'); }
function getInicioMes() { var d = new Date(); return d.getFullYear() + '-' + (d.getMonth()+1).toString().padStart(2,'0') + '-01'; }
function getNombreCliente(numeroCliente) { var cliente = todosLosClientes.find(function(c) { return c.numero == numeroCliente; }); return cliente ? cliente.nombre : 'Cliente #' + numeroCliente; }

async function recargarDatosAdmin() { await cargarDatosAdmin(); alert('Datos recargados - Total ventas: ' + todasLasVentas.length); }
...
async function login() {
    ...
    if (numero.toLowerCase() === 'admin') {
        if (password === ADMIN_PASSWORD) { isAdmin = true; await cargarDatosAdmin(); showScreen(adminScreen); }
    ...
    try {
        ...
        await cargarVentas(); mostrarDatosCliente(); showScreen(mainScreen);
...
async function cargarVentas() {
    ...
    try { var resp = await supabase.from('ventas').select('*').eq('numero_cliente', clienteActual.numero).order('fecha', { ascending: false }); if (!resp.error && resp.data) ventasCliente = resp.data; } catch (err) {}
}
...
function renderizarListas() {
    ...
    else { listaEnCaja.style.display = 'flex'; emptyCaja.style.display = 'none';
        var html = '';
        for (var i = 0; i < pendientes.length; i++) { var v = pendientes[i]; html += '<div class="item-card"><div class="item-info"><div class="item-name">' + (v.descripcion || 'Articulo') + '</div><div class="item-date">' + formatDateFull(v.fecha) + '</div></div><div class="item-status"><span class="item-price">' + formatMoney(v.precio) + '</span><span class="status-badge pendiente">Pendiente</span></div></div>'; }
    ...
    else { listaHistorial.style.display = 'flex'; emptyHistorial.style.display = 'none';
        var html = '';
        for (var i = 0; i < enviados.length; i++) { var v = enviados[i]; html += '<div class="item-card"><div class="item-info"><div class="item-name">' + (v.descripcion || 'Articulo') + '</div><div class="item-date">' + formatDateFull(v.fecha) + '</div></div><div class="item-status"><span class="item-price">' + formatMoney(v.precio) + '</span><span class="status-badge enviado">Enviado</span></div></div>'; }
        listaHistorial.innerHTML = html;
    }
}

function logout() { clienteActual = null; ventasCliente = []; isAdmin = false; localStorage.removeItem('clienteNumero'); localStorage.removeItem('clientePassword'); inputNumero.value = ''; inputPassword.value = ''; showScreen(loginScreen); }

function initTabs() { var tabs = document.querySelectorAll('.tab'); for (var i = 0; i < tabs.length; i++) { tabs[i].onclick = function() { var tabId = this.getAttribute('data-tab'); document.querySelectorAll('.tab').forEach(function(t) { t.classList.remove('active'); }); this.classList.add('active'); document.querySelectorAll('.tab-panel').forEach(function(p) { p.classList.remove('active'); }); document.getElementById('tab-' + tabId).classList.add('active'); }; } }

function initAdminTabs() { var tabs = document.querySelectorAll('.admin-tab'); for (var i = 0; i < tabs.length; i++) { tabs[i].onclick = function() { var tabId = this.getAttribute('data-admin-tab'); if (!tabId) return; document.querySelectorAll('.admin-tab').forEach(function(t) { t.classList.remove('active'); }); this.classList.add('active'); document.querySelectorAll('.admin-panel').forEach(function(p) { p.classList.remove('active'); }); document.getElementById('admin-' + tabId).classList.add('active'); }; } }

async function checkAutoLogin() { var savedNumero = localStorage.getItem('clienteNumero'); var savedPassword = localStorage.getItem('clientePassword'); if (savedNumero) { inputNumero.value = savedNumero; if (savedPassword) inputPassword.value = savedPassword; await login(); } }
...
async function guardarNuevaPassword() {
    ...
    try { await supabase.from('clientes').update({ password: password }).eq('numero', clienteActual.numero); clienteActual.password = password; localStorage.setItem('clientePassword', password); cerrarPopupPassword(); } catch (err) { errorMsg.textContent = 'Error'; }
}
...
async function cargarDatosAdmin() {
    ...
    try {
        ...
        while (hasMore) {
            ...
            if (resp.data && resp.data.length > 0) { allVentas = allVentas.concat(resp.data); offset += pageSize; hasMore = resp.data.length === pageSize; }
            else { hasMore = false; }
        }
        todasLasVentas = allVentas;
        console.log('Total ventas cargadas:', todasLasVentas.length);
        actualizarDashboard(); mostrarListaClientes(); mostrarListaVentas();
...
function actualizarDashboard() {
    var hoy = getHoy(); var ayer = getAyer(); var inicioSemana = getInicioSemana(); var inicioMes = getInicioMes();
    console.log('Fechas - Hoy:', hoy, 'Inicio Semana:', inicioSemana, 'Inicio Mes:', inicioMes);
    ...
    console.log('Ventas HOY:', ventasHoy.length, 'SEMANA:', ventasSemana.length, 'MES:', ventasMes.length);
    ...
    var ventasManana = ventasHoy.filter(function(v) { if (!v.hora) return false; var h = parseInt(v.hora.split(':')[0]); return h >= 10 && h < 16; });
    var ventasTarde = ventasHoy.filter(function(v) { if (!v.hora) return false; var h = parseInt(v.hora.split(':')[0]); return h >= 16 && h <= 23; });
    ...
    ranking.sort(function(a, b) { return b.total - a.total; }); ranking = ranking.slice(0, 5);
    var topHtml = '';
...
function mostrarListaClientes() { var html = ''; for (var i = 0; i < todosLosClientes.length; i++) { var c = todosLosClientes[i]; var bono = (parseFloat(c.bono_total) || 0) - (parseFloat(c.bono_gastado) || 0); html += '<div class="cliente-mini" onclick="buscarClientePorNumero(' + c.numero + ')"><span class="cliente-mini-num">#' + c.numero + '</span><span class="cliente-mini-nombre">' + (c.nombre || 'Sin nombre') + '</span><span class="cliente-mini-bono">' + formatMoney(bono) + '</span></div>'; } document.getElementById('lista-todos-clientes').innerHTML = html || '<p style="color:#888;">No hay clientes</p>'; }
...
function buscarCliente() { var texto = document.getElementById('admin-buscar-cliente').value.trim().toLowerCase(); if (!texto) return; var cliente = todosLosClientes.find(function(c) { return c.numero == texto || (c.nombre && c.nombre.toLowerCase().indexOf(texto) >= 0); }); if (cliente) mostrarClienteParaEditar(cliente); else { document.getElementById('cliente-resultado').style.display = 'none'; alert('No encontrado'); } }
function mostrarClienteParaEditar(cliente) { clienteEditando = cliente; document.getElementById('cliente-resultado').style.display = 'block'; document.getElementById('edit-cliente-num').textContent = '#' + cliente.numero; document.getElementById('edit-cliente-nombre').textContent = cliente.nombre || 'Sin nombre'; document.getElementById('edit-nombre').value = cliente.nombre || ''; document.getElementById('edit-bono-total').value = cliente.bono_total || 0; document.getElementById('edit-bono-gastado').value = cliente.bono_gastado || 0; document.getElementById('edit-password').value = cliente.password || ''; document.getElementById('edit-cliente-msg').innerHTML = ''; }
async function guardarCambiosCliente() { if (!clienteEditando) return; try { await supabase.from('clientes').update({ nombre: document.getElementById('edit-nombre').value.trim(), bono_total: parseFloat(document.getElementById('edit-bono-total').value) || 0, bono_gastado: parseFloat(document.getElementById('edit-bono-gastado').value) || 0, password: document.getElementById('edit-password').value.trim() || null }).eq('numero', clienteEditando.numero); document.getElementById('edit-cliente-msg').innerHTML = '<div class="msg success">Guardado</div>'; await cargarDatosAdmin(); } catch (err) { document.getElementById('edit-cliente-msg').innerHTML = '<div class="msg error">Error</div>'; } }
...
function mostrarListaVentas() {
    ...
    for (var i = 0; i < Math.min(ventas.length, 200); i++) {
        ...
        html += '<div class="venta-admin-item"><span class="venta-fecha">' + formatDate(v.fecha) + ' ' + hora + '</span><span class="venta-cliente">' + nombreCliente + '</span><span class="venta-precio">' + formatMoney(v.precio) + '</span><button class="venta-delete" onclick="borrarVentaAdmin(' + v.id + ')">🗑️</button></div>';
    }
    document.getElementById('ventas-lista').innerHTML = html || '<p style="color:#888;padding:20px;">No hay ventas</p>';
}

async function borrarVentaAdmin(ventaId) { if (!confirm('Borrar esta venta?')) return; try { await supabase.from('ventas').delete().eq('id', ventaId); todasLasVentas = todasLasVentas.filter(function(v) { return v.id !== ventaId; }); mostrarListaVentas(); actualizarDashboard(); } catch (err) { alert('Error'); } }
